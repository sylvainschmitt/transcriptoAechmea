...
```{r}
include_graphics("results/quality/ptr/Neo_goel.rep_compare_bars.svg")
```

```{r}
include_graphics("results/quality/ptr/Neo_goel.rep_compare_cor.svg")
```

### *Camponotus femoratus* replicates

```{r}
include_graphics("results/quality/ptr/Cam_femo.rep_compare_bars.svg")
```

```{r}
include_graphics("results/quality/ptr/Cam_femo.rep_compare_cor.svg")
```
...
```{r}
include_graphics("results/quality/ptr/RSEM.isoform.TMM.EXPR.matrix.minRow10.CPM.log2.sample_cor_matrix.svg")
```

```{r}
include_graphics("results/quality/ptr/RSEM.isoform.TMM.EXPR.matrix.minRow10.CPM.log2.centered.prcomp.principal_components.svg")
```
...
```{r de, caption="Volcano plot."}
f <- 2
p <- 0.001
de <- vroom("results/expression/voom/RSEM.isoform.TMM.EXPR.matrix.Cam_femo_vs_Neo_goel.voom.DE_results",
...
```{r}
include_graphics("results/expression/voom/analysis/diffExpr.P1e-3_C2.matrix.log2.centered.genes_vs_samples_heatmap.svg")
```
