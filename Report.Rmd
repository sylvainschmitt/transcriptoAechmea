...
One metric for evaluating the quality of a transcriptome assembly is to examine the number of transcripts that were assembled that appear to be full-length or nearly full-length. A general analysis to perform is to align the assembled transcripts against all known proteins and to determine the number of unique top matching proteins that align across more than X% of its length.

```{r fulllength, fig.cap="Caption."}
# read_tsv("results/quality/full_length/blastx.outfmt6.txt.w_pct_hit_length")
```
...
*Methodology from [rnaQUAST](https://github.com/ablab/rnaquast).*
